...
1. Для балансировки трафика нам нужен специальное ПО, собственно балансировщик. Например, [HAProxy](https://www.haproxy.org/) — добавляем его в `docker-compose.yml`:
   ...
   В раздел `services` нам нужно добавить этот кусочек кода.
...
3. Перезапускаем проект, проверяем что:
   ...
   - Обе ноды корректно поднялись и распределили между собой роли мастера и слейва (_аналогично Пункту 4 Части 1_)
   - Зукипер подцепился к кластеру без ошибок (_аналогично Пункту 4 Части 1_)
   - Хапрокси поднялась без ошибок

   ####

   Перезапускаем кластер. Сначала останавливаем его командой `docker-compose down`:

   ![img_12.png](img_12.png)

   Затем запускаем заново командой `docker-compose up -d`:

   ![img_13.png](img_13.png)

   Проверим статус HAProxy командой `docker-compose logs haproxy`:
...
4. Аналогично Пункту 1 Части 2, подключаемся к “новой” базе данных, в качестве адреса используя имя контейнера с HAProxy. Проверяем, что подключение успешное - по умолчанию оно всегда будет редиректить трафик на мастер-ноду
   ...
   Подключаемся через HAProxy командой `psql -h localhost -p 5432 -U postgres -d postgres`:

   ![img_15.png](img_15.png)

   Проверяем, к какой ноде я подключился:

   ![img_16.png](img_16.png)

   Ну да, тут редирект на мастер-ноду, потому что вывод `f`, т.е. можно записывать данные.

### Задание

Улучшить лабораторную, настроив кластер так, чтобы после “возвращения” второй ноды в кластер она автоматически получала данные, которые
были записаны в ее отсутствие

####

Нужно переписать конфигурационные файлы `postgres0.yml` и `postgres1.yml`. В секции `postgresql` нужно добавить параметры:
...
`pg_rewind` - это утилита PostgreSQL. Если отключенная нода успела что-то записать у себя "в отрыве" от мастера (это создаст новый таймлайн), то после возвращения `pg_rewind` не будет копировать всю базу заново. Вместо этого она "отмотает" изменения на отставшей ноде до точки расхождения и затем применит WAL-журналы с мастера. Это намного быстрее полной пересинхронизации.

Настройка `recovery_conf` указывает вернувшейся ноде, где искать архив WAL-журналов (`/tmp/wal_archive/`), чтобы восстановить пропущенные изменения, и следовать за последним (`latest`) таймлайном.

Ну и параметры WAL из `bootstrap.dcs.postgresql.parameters` перенесем в `postgresql.parameters`, т.к. в последнем случае у нас основные, рабочие параметры для всех нод в кластере, которые применяются всегда и имеют высший приоритет.

После этого перезапустим кластер:

![img_17.png](img_17.png)

Теперь проверим ноды. Мастер:
...
Вставим данные в мастер-ноду:

![img_20.png](img_20.png)

Теперь отключаем вторую ноду командой `docker-compose stop pg-slave`:

![img_21.png](img_21.png)

Опять запишем данные в мастер-ноду:

![img_22.png](img_22.png)

Возвращаем реплику в кластер командой `docker-compose start pg-slave`:

![img_23.png](img_23.png)

Теперь посмотрим на реплике данные:

![img_24.png](img_24.png)

Как мы видим, у нас на рплике появились данные, которые мы записали в мастер-ноду, пока она была отключена. Это значит, что все злорово работает.

## Выводы

Очень крутая лаба, спасибо за понятный гайд, но все равно было над чем подумать и что погуглить